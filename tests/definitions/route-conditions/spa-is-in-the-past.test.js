...
const spaIsInThePast = require('../../../definitions/route-conditions/spa-is-in-the-past.js');

describe('route-conditions/spa-is-in-the-past', () => {
  ...
  it('should be a function', () => {
    expect(spaIsInThePast).to.be.a('function');
  });

  it('should return false if DOB has no page data', () => {
    const context = new JourneyContext({});
    return expect(spaIsInThePast(stubRoute, context)).to.be.false;
  });
  ...
  it('should return true if claimant is over State Pension Age', () => {
    const dateOfBirth = { yyyy: '1920', mm: '01', dd: '01' };
    const context = new JourneyContext({ test: { dateOfBirth } });
    return expect(spaIsInThePast(stubRoute, context)).to.be.true;
  });
