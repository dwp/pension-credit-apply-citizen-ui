...
const {
  encrypt, decrypt, KeyringNode, NodeCachingMaterialsManager, AlgorithmSuiteIdentifier,
...
class RedisEncrypter extends Redis {
  /**
   * The `cipherSuite` must be a valid suite chosen from the SDK. See those
   * listed in:
   * https://github.com/aws/aws-encryption-sdk-javascript/blob/master/modules/material-management/src/algorithm_suites.ts#L24-L33
   *
   ...
   * @param {number} cipherSuite The encryption algorithm suite to use
   */
  constructor(
    host,
    config,
    keyring = null,
    encryptionContext = {},
    cipherSuite = AlgorithmSuiteIdentifier.ALG_AES256_GCM_IV12_TAG16_HKDF_SHA384_ECDSA_P384,
  ) {
    if (
      keyring !== null
    ...
    if (!validCipherSuiteIds.includes(cipherSuite)) {
      throw new ReferenceError('cipherSuite must be a valid AWS SDK suite identifier');
    }
   ...
   */
  set(args, cb) {
    const { keyring, encryptionContext, cipherSuite } = pryvate.get(this);
