...
const ConfigIngestor = (config = {}) => {
  const parsed = {
    ...
    TIMEOUT_DIALOG_COUNTDOWN: config.TIMEOUT_DIALOG_COUNTDOWN
      && parseInt(config.TIMEOUT_DIALOG_COUNTDOWN, 10),
    ...
    AGGRESSIVE_ASSET_CACHING: config.AGGRESSIVE_ASSET_CACHING === 'true',
  };
  ...
  // Default TIMEOUT_DIALOG_COUNTDOWN to 5 minutes before session expiry if not
  // configured and SESSION_TLL greater than 5 minutes.
  if (!parsed.TIMEOUT_DIALOG_COUNTDOWN) {
    parsed.TIMEOUT_DIALOG_COUNTDOWN = parsed.SESSION_TTL > 300 ? parsed.SESSION_TTL - 5 * 60 : null;
  }

  if (typeof parsed.TIMEOUT_DIALOG_COUNTDOWN !== 'number' || Number.isNaN(parsed.HTTP_TIMEOUT)) {
    throw new TypeError(`TIMEOUT_DIALOG_COUNTDOWN must be a positive integer. Given ${String(config.HTTP_TIMEOUT)}`);
  }

  if (!parsed.DISABLE_SESSION_TTL_RESTRICTIONS) {
    if (parsed.TIMEOUT_DIALOG_COUNTDOWN > 3600 || parsed.TIMEOUT_DIALOG_COUNTDOWN < 60) {
      throw new RangeError('TIMEOUT_DIALOG_COUNTDOWN must be within the range 60 to 3600 seconds');
    }

    if (!parsed.DISABLE_SESSION_TTL_RESTRICTIONS && parsed.TIMEOUT_DIALOG_COUNTDOWN % 60 > 0) {
      throw new Error('TIMEOUT_DIALOG_COUNTDOWN must be a factor of 60');
    }
  }

  if (parsed.TIMEOUT_DIALOG_COUNTDOWN >= parsed.SESSION_TTL) {
    throw new RangeError('TIMEOUT_DIALOG_COUNTDOWN must be less than SESSION_TTL');
  }
