...
module.exports = (
  ...
  encryptionContextTag = undefined,
) => {
  ...
  if (redisHosts.length > 0) {
    ...
    const encryptionContext = encryptionContextTag ? {
      tag: encryptionContextTag,
    } : undefined;

    // Setup the Redis session store
    logger.info('Initialising Redis session store (cluster mode = %s, cipher suite = %s)', useClusterMode ? 'on' : 'off', cipherSuiteId);
    const RedisStore = connectRedis(session);
    const redisClient = useClusterMode
      ? new RedisEncrypter.Cluster(
        redisHosts,
        redisClusterConfig,
        cryptoKeyring,
        encryptionContext,
